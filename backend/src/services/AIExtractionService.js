...
//       INPUT DATA is JSON/Excel. The keys are column headers.
...
//       - Map "Product Name", "Item Description", "Particulars" -> 'productName'
//       - Map "Total Amount", "Net Amount", "Grand Total" -> 'invoiceGrandTotal'
//       - Map "Date", "Invoice Date" -> 'date'

//       ### STEP 2: CATEGORIZATION
//       - 'product' = Physical Goods (e.g. Glass, Phone, Rice)
//       - 'service' = Fees, Charges, Tax, Shipping, Adjustments.
//       - If 'productName' is missing in the file, use "General Invoice Item" (type='service').
...
export class AIExtractionService {
  ...
  static normalizeData(rawData) {
    ...
    invoices.forEach((item, index) => {
        ...
        if (!serial || String(serial).toLowerCase() === 'null' || String(serial).trim() === '') {
            // ✅ FIX: Unique Fallback per row to prevent false merging
            serial = `MISSING-INV-${index}-${Math.floor(Math.random() * 1000)}`;
        } else {
            serial = String(serial).trim();
        }
